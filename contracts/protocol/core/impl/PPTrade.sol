...
import { I_PPTrader } from "../intf/I_PPTrader.sol";
 ...
 *  I_PPTrader interface and approved as global operators on the PowerPerpetual contract.
 */
contract PPTrade is
    PPFinalSettlement
{
        ...
        nonReentrant
    {
        ...
        PPTypes.Balance[] memory initialBalances = _settleAccounts(context, accounts);
        PPTypes.Balance[] memory currentBalances = new PPTypes.Balance[](initialBalances.length);
        ...
        for (i = 0; i < trades.length; i++) {
            ...
            PPTypes.Balance memory makerBalance = currentBalances[tradeArg.makerIndex];
            PPTypes.Balance memory takerBalance = currentBalances[tradeArg.takerIndex];
    ...
    function _verifyAccountsFinalBalances(
        ...
        address[] memory accounts,
        PPTypes.Balance[] memory initialBalances,
        PPTypes.Balance[] memory currentBalances
    )
        private
        pure
    {
        for (uint256 i = 0; i < accounts.length; i++) {
            PPTypes.Balance memory currentBalance = currentBalances[i];
            (uint256 currentPos, uint256 currentNeg) =
                currentBalance.getPositiveAndNegativeValue(context.price);

            // See PPSettlement._isCollateralized().
            ...
            address account = accounts[i];
            PPTypes.Balance memory initialBalance = initialBalances[i];
