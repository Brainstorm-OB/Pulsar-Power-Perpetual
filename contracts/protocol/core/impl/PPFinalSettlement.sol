...
import { PPSettlement } from "./PPSettlement.sol";
    ...
    PPSettlement
{
    ...
    */
        ...
        nonReentrant
    {
        ...
        PPTypes.Context memory context = PPTypes.Context({
        ...
        PPTypes.Balance memory balance = _settleAccount(context, msg.sender);

        // Determine the account net value.
        // `positive` and `negative` are base values with extra precision.
        (uint256 positive, uint256 negative) = PPBalanceMath.getPositiveAndNegativeValue(
            balance,
            context.price
        );
        ...
        // Get the number of tokens in the PowerPerpetual Contract.
