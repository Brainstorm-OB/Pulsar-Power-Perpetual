 ...
 *  This is to be used with inverse power perpetuals, where the base currency is the margin currency used
 *  by the PowerPerpetual smart contract, and the quote currency is the “position” currency.
 */
contract PPInverseOrders is
    PPTraderConstants
{
    ...
    string constant private EIP712_DOMAIN_NAME = "PPInverseOrders";
        ...
        returns (PPTypes.TradeResult memory)
    {
        ...
        if (taker != sender) {
            require(
                PPGetters(perpetual).hasAccountPermissions(taker, sender),
        ...
        // Inverse power perpetual: The fill amount is denoted in quote currency (position).
        emit LogOrderFilled(
            orderHash,
        ...
        // Inverse power perpetual: Note that `isBuy` in the order is from the maker's perspective and
        ...
        view
    {
        ...
        // Inverse power perpetual: The trigger price must be compared against the inverted oracle price.
        ...
        if (_isDecreaseOnly(tradeData.order)) {
            PPTypes.Balance memory balance = PPGetters(perpetual).getAccountBalance(maker);
            // Inverse power perpetual: Buying the base currency means selling position and selling the
