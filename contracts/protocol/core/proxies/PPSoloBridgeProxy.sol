...
import { I_Solo } from "../../../external/pulsar/I_Solo.sol";
...
/**
 * @title PPSoloBridgeProxy
 * @author Pulsar
 *
 * @notice Facilitates transfers between the PowerPerpetual and Solo smart contracts.
 */
contract PPSoloBridgeProxy is
    PPProxy,
    ReentrancyGuard
{
    ...
    string constant private EIP712_DOMAIN_NAME = "PPSoloBridgeProxy";
    ...
    /**
     * @notice Executes a transfer from Solo to PowerPerpetual or vice vera.
        ...
        returns (uint256)
    {
        ...
        I_PowerPerpetual perpetual = I_PowerPerpetual(transfer.perpetual);
        address tokenAddress = perpetual.getTokenContract();
        TransferMode transferMode = _getTransferMode(transfer);
        bool toPowerPerpetual = (
            transferMode == TransferMode.SomeToPowerPerpetual ||
            transferMode == TransferMode.AllToPowerPerpetual
        );
        ...
        returns (bool)
    {
        ...
        // permissions for either transfer direction. Only require PowerPerpetual operator permissions to
        // withdraw from PowerPerpetual.
        return
            isSoloOperator &&
            (toPowerPerpetual || perpetual.hasAccountPermissions(transfer.account, msg.sender));
    }
    ...
    function _verifySoloMarket(
        ...
        view
    {
        // Verify that the Solo market asset matches the PowerPerpetual margin asset.
        require(
            solo.getMarketTokenAddress(transfer.soloMarketId) == tokenAddress,
            "Solo and PowerPerpetual assets are not the same"
        );
    }
