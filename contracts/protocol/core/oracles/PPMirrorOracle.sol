...
/**
 * @title PPMirrorOracle
 * @author Pulsar
 *
 * Oracle which mirrors an underlying oracle.
 */
contract PPMirrorOracle is
    Ownable,
    I_MakerOracle
{
    // ============ Events ============
        ...
        returns (bytes32, bool)
    {
        require(
            ...
            "PPMirrorOracle#peek: Sender not authorized to get price"
        );
        uint256 val = _VAL_;
        ...
        returns (bytes32)
    {
        require(
            _READERS_[msg.sender] == 1,
            "PPMirrorOracle#read: Sender not authorized to get price"
        );
        uint256 val = _VAL_;
        require(
            val > 0,
            "PPMirrorOracle#read: Price is zero"
        );
        return bytes32(val);
    }
    ...
    function poke(
        ...
        external
    {
        require(val_.length == _BAR_, "PPMirrorOracle#poke: Wrong number of messages");
        ...
        for (uint256 i = 0; i < val_.length; i++) {
            ...
            require(_ORCL_[signer] == 1, "PPMirrorOracle#poke: Invalid signer");

            // Verify that the message is newer than the last oracle update.
            require(age_i > zzz, "PPMirrorOracle#poke: Stale message");

            // Verify that the messages are ordered by value.
            require(val_i >= last, "PPMirrorOracle#poke: Message out of order");
            last = val_i;
            ...
            uint256 signerBit = uint256(1) << signerId;
            require(bloom & signerBit == 0, "PPMirrorOracle#poke: Duplicate signer");
        ...
        external
    {
        for (uint256 i = 0; i < signers.length; i++) {
            ...
            require(
                ...
                "PPMirrorOracle#lift: Signer not authorized on underlying oracle"
            );
            ...
            require(
                ...
                "PPMirrorOracle#lift: Signer already authorized"
            );
        ...
        external
    {
        for (uint256 i = 0; i < signers.length; i++) {
            ...
            require(
                ...
                "PPMirrorOracle#drop: Signer is authorized on underlying oracle"
            );
            ...
            require(
                _ORCL_[signer] != 0,
                "PPMirrorOracle#drop: Signer is already not authorized"
            );
